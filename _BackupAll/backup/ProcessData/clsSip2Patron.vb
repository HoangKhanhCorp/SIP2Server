...
Public Class clsSip2Patron
    ...
    Private Function CutVietnameseAccent(ByVal strInputs As String) As String
        ...
        For inti = 0 To strInputs.Length - 1
            ...
            If InStr("A,À,Á,Ả,Ã,Ạ,Â,Ầ,Ấ,Ẩ,Ẫ,Ậ,Ă,Ằ,Ắ,Ẳ,Ẵ,Ặ,A,Ã€,Ã,áº¢,Ãƒ,áº ,Ã‚,áº¦,áº¤,áº¨,áºª,áº¬,Ä‚,áº°,áº®,áº²,áº´,áº¶", strInput) > 0 Then
                strNoAccentChar = "A"
            ElseIf InStr("a,à,á,ả,ã,ạ,â,ầ,ấ,ẩ,ẫ,ậ,ă,ằ,ắ,ẳ,ẵ,ặ,a,Ã ,Ã¡,áº£,Ã£,áº¡,Ã¢,áº§,áº¥,áº©,áº«,áº­,Äƒ,áº±,áº¯,áº³,áºµ,áº·", strInput) > 0 Then
                strNoAccentChar = "a"
            ElseIf InStr("E,È,É,Ẻ,Ẽ,Ẹ,Ê,Ề,Ế,Ể,Ễ,Ệ,E,E,Ãˆ,Ã‰,áºº,áº¼,áº¸", strInput) > 0 Then
                strNoAccentChar = "E"
            ElseIf InStr("e,è,é,ẻ,ẽ,ẹ,ê,ề,ế,ể,ễ,ệ,e,Ã¨,Ã©,áº»,áº½,áº¹,Ãª,á»,áº¿,á»ƒ,á»…,á»‡", strInput) > 0 Then
                strNoAccentChar = "e"
            ElseIf InStr("U,Ù,Ú,Ủ,Ũ,Ụ,Ư,Ừ,Ứ,Ử,Ữ,Ự,U,Ã™,Ãš,á»¦,Å¨,á»¤,Æ¯,á»ª,á»¨,á»¬,á»®,á»°", strInput) > 0 Then
                strNoAccentChar = "U"
            ElseIf InStr("u,ù,ú,ủ,ũ,ụ,ư,ừ,ứ,ử,ữ,ự,u,Ã¹,Ãº,á»§,Å©,á»¥,Æ°,á»«,á»©,á»­,á»¯,á»±", strInput) > 0 Then
                strNoAccentChar = "u"
            ElseIf InStr("I,Ì,Í,Ỉ,Ĩ,Ị", strInput) > 0 Then
                strNoAccentChar = "I"
            ElseIf InStr("i,ì,í,ỉ,ĩ,ị", strInput) > 0 Then
                strNoAccentChar = "i"
            ElseIf InStr("O,Ò,Ó,Ỏ,Õ,Ọ,Ô,Ồ,Ố,Ổ,Ỗ,Ộ,Ơ,Ờ,Ớ,Ở,Ỡ,Ợ,O,Ã’,á»Ž,Ã•,á»Œ", strInput) > 0 Then
                strNoAccentChar = "O"
            ElseIf InStr("o,ò,ó,ỏ,õ,ọ,ô,ồ,ố,ổ,ỗ,ộ,ơ,ờ,ớ,ở,ỡ,ợ,o,Ã²,Ã³,á»,Ãµ,á»,Ã´,á»‘,á»•,á»—,á»™,Æ¡,á»,á»›,á»Ÿ,á»¡,á»£", strInput) > 0 Then
                strNoAccentChar = "o"
            ElseIf InStr("Y,Ỳ,Ý,Ỷ,Ỹ,Ỵ,Y,á»²,Ã,á»¶,á»¸,á»´", strInput) > 0 Then
                strNoAccentChar = "Y"
            ElseIf InStr("y,ỳ,ý,ỷ,ỹ,ỵ,y,á»³,Ã½,á»·,á»¹,á»µ", strInput) > 0 Then
                strNoAccentChar = "y"
    ...
    Private Sub LoadFields(ByVal arrLabels() As String, ByRef colf As Collection, ByVal req As String)
        ...
        For i = 0 To UBound(strPairs)
            strFieldID = Left(strPairs(i), 2)
            strFieldVal = Right(strPairs(i), Len(strPairs(i)) - 2)
            Try
    ...
    Private Function GetContent(ByVal strCopynumber As String) As DataTable
        '   Dim tblContent As DataTable = GetData("SELECT A.Content,B.Note,D.Code,C.Symbol from Field200s A,HOLDING B, Holding_Location C, Holding_library D where A.ItemID=B.ItemID and Upper(B.CopyNumber)='" & strCopynumber.ToUpper & "' and A.FieldCode = '245' And B.LocationID=C.ID and C.LibID=D.ID")
        Dim tblContent As DataTable = GetData("SELECT A.Gia_tri as Content,B.Note,D.Ten_viet_tat as Code,C.Ten_viet_tat_kho as Symbol from Field200s A,Ma_xep_gia B, Kho C, Thu_vien D where A.Tai_lieu_id=B.Tai_lieu_id and Upper(B.Ma_xep_gia)='" & strCopynumber.ToUpper & "' and A.Truong_ID = 448 And B.Kho_ID=C.Kho_ID and C.Thu_vien_ID=D.Thu_vien_ID")
    ...
    Public Function Renew(ByVal req As String) As String
        ...
        If UCase(strNoBlock) = "Y" Then
            ...
            If Not colFields("AB") = "" Then
                If Not strDBServer = "ORACLE" Then
                    Me.ExecuteQuery("UPDATE CIR_LOAN SET RenewCount = RenewCount + 1, DueDate = '" & ConvertDateBack(dtmNewDueDate) & "' WHERE PatronID IN (SELECT ID FROM CIR_PATRON WHERE Code = '" & colFields("AA") & "') AND CopyNumber = '" & colFields("AB") & "'")
                Else
                    Me.ExecuteQuery("UPDATE CIR_LOAN SET RenewCount = RenewCount + 1, DueDate = ConvertDate('" & ConvertDateBack(dtmNewDueDate) & "') WHERE PatronID IN (SELECT ID FROM CIR_PATRON WHERE Code = '" & colFields("AA") & "') AND CopyNumber = '" & colFields("AB") & "'")
                End If
            ...
            ElseIf Not colFields("AJ") = "" Then
                ...
                Else
                    Me.ExecuteQuery("UPDATE CIR_LOAN SET RenewCount = RenewCount + 1, DueDate = ConvertDate('" & ConvertDateBack(dtmNewDueDate) & "') WHERE PatronID IN (SELECT ID FROM CIR_PATRON WHERE Code = '" & colFields("AA") & "') AND ItemID IN (SELECT ItemID FROM ITEM WHERE Code = '" & colFields("AJ") & "')")
                End If
                strOutput = "301YUY" & PrintISOTime(Now, TimeZone) & "AO" & colFields("AO") & "|AA" & colFields("AA") & "|AB" & colFields("AB") & "|AJ" & colFields("AJ") & "|AH" & PrintISOTime(dtmNewDueDate, TimeZone)
            Else
        ...
        Else
            ...
            End If
            strSelectedTabs = "CIR_PATRON, CIR_LOAN_TYPE, CIR_LOAN, HOLDING"
            ...
            If Not colFields("AB") = "" Then
                strJoinSQL = strJoinSQL & " And upper(CIR_LOAN.CopyNumber) = '" & CStr(colFields("AB")).ToUpper & "'"
            ElseIf Not colFields("AJ") = "" Then
                strSelectedTabs = strSelectedTabs & ", ITEM"
                strJoinSQL = strJoinSQL & " AND CIR_LOAN.ItemID = ITEM.ItemID AND Code = '" & colFields("AJ") & "'"
            Else
                strOutput = "300NUU" & PrintISOTime(Now, TimeZone) & "AO" & colFields("AO") & "|AA" & colFields("AA") & "|AB" & colFields("AB") & "|AJ|AH|AFNo item specified"
                Renew = strOutput
                Exit Function
    ...
    Public Function ItemInformation(ByVal req As String) As String
        ...
        If tblResult.Rows.Count > 0 Then
            ...
            End If
            Dim tblResult1 As DataTable
            tblResult1 = Me.GetData("SELECT ngay_muon DueDate, RecallDate FROM an_pham_cho_muon WHERE ma_xep_gia = '" & colFields("AB") & "'")
            If tblResult1.Rows.Count > 0 Then
                strDueDate = PrintISOTime(CDate(tblResult1.Rows(0).Item("DueDate")), TimeZone)
            ...
            End If
            tblResult1 = Nothing
            tblResult1 = Me.GetData("SELECT Thoi_diem_het_han as TimeOutDate FROM Giu_cho WHERE (Ma_xep_gia = '' OR Ma_xep_gia = '" & colFields("AB") & "') AND Tai_lieu_ID = " & dblDocID & " ORDER BY Thoi_diem_het_han")
            ...
            End If
            strOutput = "18" & strCirStatus & "00" & strFeeType & PrintISOTime(Now, TimeZone) & "AB" & colFields("AB")
            If intQueueLen > 0 Then
        ...
        If IsNumeric(colFields("BV")) Then
            If CDbl(colFields("BV")) > 0 Then
                ...
                If tblResult.Rows.Count > 0 Then
                    ...
                    Me.ExecuteQuery("UPDATE CIR_PATRON SET Debt = " & CStr(-1 * tblResult.Rows(0).Item("TotalDebt")) & " WHERE UPPER(CODE) = '" & CStr(colFields("AA")).ToUpper & "'")
    ...
    Public Function CheckIn(ByVal req As String) As String
        ...
        If intRetval <> 3 Then
            If intRetval = 9 Then
                ...
                Else
                    tblResult = Me.GetData("select tai_lieu_id as ItemID,InUsed from ma_xep_gia where upper(ma_xep_gia)='" & CStr(colFields("AB")).ToUpper & "'")
                    If Not tblResult.Rows(0).Item("InUsed") = 0 Then
                        ...
                        Me.ExecuteQuery("UPDATE ma_xep_gia SET InUsed = 0 WHERE upper(ma_xep_gia)='" & CStr(colFields("AB")).ToUpper & "'")
                    End If
    ...
    Public Function SCStatus(ByVal req As String) As String
        SCStatus = "98YYYYYN300005" & PrintISOTime(Now, TimeZone) & "2.00AO|AM|BXYYYYYYYYYYYYYYYY|AN|AF|AG"
    End Function
    ...
    Public Function CheckOut(ByVal req As String) As String
        ...
        Else
            ...
            tblResult = Me.GetData("SELECT ID FROM Ban_doc WHERE Upper(So_the) = '" & strPatronCode.ToUpper & "'")
            If Not tblResult.Rows.Count = 0 Then
    ...
    Public Function PatronStatusRequest(ByVal req As String, ByVal blnPWDRequired As Boolean, Optional ByVal strMesage As String = "") As String
        ...
        tblResult = Me.GetData("SELECT so_the as Code,Ho_ten as Fullname,Mat_khau as Password,Debt FROM Ban_doc WHERE Upper(So_the) = '" & Replace(colFields("AA"), "'", "''").ToUpper & "'")
        If tblResult.Rows.Count = 0 Then
    ...
    Public Function PatronInformation(ByVal req As String) As String
        ...
        Else
            ...
            Else
                curDebt = CDbl(tblResult.Rows(0).Item("Debt"))
                ...
                tblTempCounts = Me.GetData("SELECT CopyNumber, Content FROM CIR_HOLDING, Field200s WHERE CIR_HOLDING.ItemID = Field200s.ItemID AND FieldCode = '245' AND UPPER(PatronCode) = UPPER('" & strPatronCode & "') ORDER BY CIR_HOLDING.ID")
                intUHoldCounts = tblTempCounts.Rows.Count
                ...
                If Not tblTempCounts.Rows.Count = 0 Then
                    ...
                    If InStr(strSummary, "Y") = 1 Then
                        ...
                        End If
                    End If
                    tblTempCounts = Me.GetData(strSqlCounts & " and RecallDate is not null ORDER BY CIR_LOAN.ID")
                    ...
                    tblTempCounts = Me.GetData(strSqlCounts & " and Fee > 0 ORDER BY CIR_LOAN.ID")
                    ...
                    Else
                        tblTempCounts = Me.GetData(strSqlCounts & " and convert(varchar,DueDate,103) < convert(varchar,getdate(),103) ORDER BY CIR_LOAN.ID")
                    End If
                        ...
                        If intStartPos < tblTempCounts.Rows.Count And intStartPos > 0 Then
                            For i = intStartPos To intEndPos
                                ...
                                If InStr(strTitle, "/") > 0 Then
                                    strTitle = Left(strTitle, InStr(strTitle, "/") - 1)
                                End If
                                strItemDetails = strItemDetails & "|AT" & strTitle & ". Item ID: " & tblTempCounts.Rows(i).Item("Content")
                            Next
                        End If
                    End If
                    If strDBServer = "ORACLE" Then
                    ...
                    Else
                        tblTempCounts = Me.GetData(strSqlCounts & " and convert(varchar,DueDate,103) < convert(varchar,getdate(),103) AND OverdueFine > 0 ORDER BY CIR_LOAN.ID")
                    End If
